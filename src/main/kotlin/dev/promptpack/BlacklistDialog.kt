...
class BlacklistDialog(
  ...
  override fun createActions(): Array<Action> {
    val reset =
      object : DialogWrapperAction(PromptPackBundle.message("blacklist.reset")) {
        override fun doAction(e: ActionEvent) {
          dirsModel.removeAll()
          PromptPackDefaults.IGNORED_DIRS
            .sortedBy { it.lowercase(Locale.ROOT) }
            .forEach(dirsModel::add)
          extsModel.removeAll()
          PromptPackDefaults.IGNORED_EXTS
            .sortedBy { it.lowercase(Locale.ROOT) }
            .forEach(extsModel::add)
          filesModel.removeAll()
          PromptPackDefaults.IGNORED_FILES
            .map { it.lowercase(Locale.ROOT) }
            .sortedBy { it }
            .forEach(filesModel::add)
        }
      }
